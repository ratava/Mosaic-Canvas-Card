...
- **Label fields** — content, icon picker, styling, actions.

- **Icon fields** — Icon (the glyph, an optional entity, and per-state rules),
  Visibility (hide below a value), Text Style, Actions. Tick **Use state value**
  in the Icon section and the entity's own state names the glyph — handy for
  weather, where one entity has a glyph per condition. **Alt icon prefix** points
  that lookup at another icon set, for installs that have added Iconify
  collections through an integration such as Custom Icons.
